...
int interl(line Seg, line L) {
	return side(Seg.a, Seg.b, L) <= 0 || side(Seg.a, Seg.b, L) <= 0;
}
...
double disptol(point p, line L) {
	return myabs(cross(p - L.a, L.b - L.a) / dis(L.b, L.a));
}
/* segment */
...
{
	int i;
	while (scanf("%d", &P.size), P.size)
	{
		for (i = 0; i < P.size; i++)
			cin >> P.poly[i];
		P.poly[P.size] = P.poly[0];
		if (!P.isconvex()) puts("Yes");
		else puts("No");
	}
}